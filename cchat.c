...
int main (int argc, char **argv){
   ...
   switch (c)
     ...
     case '?':
       if (optopt == 'n' | optopt == 'h' | optopt == 'p' | optopt == 'a')
         ...
         fprintf (stderr,
                  "Caracter desconocido en opcion `\\x%x'.\n",
                  optopt);
  ...
  if (contador != 4){
      printf("Introduzca todos lo argumentos : -n (nombre) -p (puerto) -h (host) -a (archivo)\n" );
      return 0;
  }
...
// Obtener tipo de servidor
  ...
  if (inet_aton(host, &serveraddr.sin_addr)){
      hoststruct = gethostbyaddr((char *) &serveraddr.sin_addr, sizeof(serveraddr.sin_addr), AF_INET);
  }else{
  ...
  if ((connect(sockfd, (struct sockaddr *) &serveraddr, sizeof(serveraddr)) )< 0){ 
    ...
    //manejando archivo
    if (status = pthread_create(&hilos[0],NULL,recibir,(void*)&sockfd)){
